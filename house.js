...
export class House1 extends GrObject {
	constructor(params={}) {
		...
		house.position.set(params.x || 0, params.y || 0, params.z || 0);
		house.rotateY(Math.PI/2*(params.rotate||0));
		super('house1',house);
	}
}
...
export class Tree1 extends GrObject {
	constructor(params={}) {
		...
		s1.position.y=4;
		let s2g = new T.TorusKnotGeometry(1.5, 0.5, 80, 18, 2, 3);
		...
		tree.add(s2r);
		tree.add(s2rr);
		tree.position.set(params.x || 0,
			params.y || 0,
			params.z || 0);
		tree.rotateY(Math.PI/2*(params.rotate||0));
		super('tree1',tree);
	}
}
...
export class Tree2 extends GrObject {
	constructor(params={}) {
		...
		let tr = new T.Mesh(trg,trm);
		tr.position.y=2;
		let lvs = new T.MeshLambertMaterial({color:'#0d6b02'});
